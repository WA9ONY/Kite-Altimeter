#!/usr/bin/env python3
# barometerV710e.py — V7.10e (minimal changes from V7.10d)
# Changes: window title updated; Plot 2 grows from left like Plot 1.
...
PROGRAM_ID = "V710e"
VERSION_STRING = "Atmospheric Pressure Logger V7.10e / HW7 SW10e"
...
# ───── UI setup ─────
root = tk.Tk()
root.title("Barometer V710e, HW:7, SW:10e")
...
# Plot 2 (1 min per px). Now grows from the LEFT like Plot 1.
...
def draw_plot2():
    ...
    if n >= w:
        means = means[-w:]
        xs = list(range(w))       # scroll when full, like plot 1
    else:
        xs = list(range(n))       # grow from LEFT
...
def update():
    ...
    # labels (hPa suffix after numbers)
