#!/usr/bin/env python3
# barometerV710d.py — V7.10d
# Plot 1: 1 px = 1 sample strip-chart (raw + MA + NOAA60)
# Plot 2: 1 px = 1 minute, each pixel = average of 60 samples (one red trace). Hour ticks every 60 px.
...
PROGRAM_ID = "V710d"
VERSION_STRING = "Atmospheric Pressure Logger V7.10d / HW7 SW10d"

SAMPLE_INTERVAL = 1.0           # seconds (1 Hz)
STRIP_HISTORY = []              # per-sample history for Plot 1 and Plot 2 aggregation
SMOOTH_WINDOW = 10              # user-adjustable smoothing window (samples) for Plot 1
NOAA_SMOOTH_N = 60              # fixed 60-sample mean for Plot 1
...
# ───── UI setup ─────
root = tk.Tk()
# Keep previous title text unless explicitly changed
root.title("Barometer V710c, HW:7, SW:10c")
root.geometry("700x950+0+0")   # enlarged default size so both plots (300px each) fit comfortably
...
noaa_label = tk.Label(L, text="NOAA Smoothed: -- hPa", font=DATA, bg=ROOT_BG, fg="green", anchor="w"); noaa_label.grid(row=1, column=0, sticky="w")
sm_lbl   = tk.Label(L, text="Smoothed: -- hPa", font=DATA,   bg=ROOT_BG, fg="blue", anchor="w");  sm_lbl.grid(row=2, column=0, sticky="w")
tr_lbl   = tk.Label(L, text="Trend: -- hPa/min", font=DATA, bg=ROOT_BG, anchor="w");  tr_lbl.grid(row=3, column=0, sticky="w")
nz_lbl   = tk.Label(L, text="Noise (RMS): -- hPa", font=DATA, bg=ROOT_BG, anchor="w");  nz_lbl.grid(row=4, column=0, sticky="w")
abs_inhg = tk.Label(L, text="Absolute (inHg): --", font=DATA,   bg=ROOT_BG, anchor="w");  abs_inhg.grid(row=5, column=0, sticky="w")
...
# ───── Plots ─────
plots_frame = tk.Frame(main, bg=ROOT_BG)
plots_frame.grid(row=3, column=0, columnspan=2, sticky="nsew")
main.grid_rowconfigure(3, weight=1)

# Plot 1
plot1_frame = tk.Frame(plots_frame, bg=ROOT_BG)
plot1_frame.pack(fill="both", expand=True)
plot1_label = tk.Label(plot1_frame, text="Plot 1: Last samples (1 px = 1 sample)", font=LABELF, bg=ROOT_BG, anchor="w")
plot1_label.pack(anchor="w")
canvas1 = tk.Canvas(plot1_frame, height=300, bg="white", highlightthickness=1, highlightbackground="black")
canvas1.pack(fill="both", expand=True, pady=(0,8))

# Plot 2 (1 min per px)
plot2_frame = tk.Frame(plots_frame, bg=ROOT_BG)
plot2_frame.pack(fill="both", expand=True)
plot2_label = tk.Label(plot2_frame, text="Plot 2: 1 px = 1 minute (mean of 60 samples)", font=LABELF, bg=ROOT_BG, anchor="w")
plot2_label.pack(anchor="w")
canvas2 = tk.Canvas(plot2_frame, height=300, bg="white", highlightthickness=1, highlightbackground="black")
canvas2.pack(fill="both", expand=True)

def _format_hpa_label(data):
    if not data: return ""
    dmin = min(data); dmax = max(data); dchg = dmax - dmin
    return f"H {dmax:.3f} hPa, L {dmin:.3f} hPa, ΔhPa: {dchg:.3f}"

def draw_plot1():
    # Plot 1: 1 px = 1 sample, grow from left to right, scroll when full
    canvas1.delete("all")
    w = max(canvas1.winfo_width(), 10); h = max(canvas1.winfo_height(), 10)
    # Grid ticks each 60 px (1 minute at 1 Hz) for context
    for x in range(0, w, 60):
        canvas1.create_line(x, 0, x, h-1, fill="#cfd8dc")

    n = len(STRIP_HISTORY)
    plot1_label.config(text="Plot 1: Last samples (1 px = 1 sample)   " + _format_hpa_label(STRIP_HISTORY))
    if n < 2: return
    ...
    if len(raw_pts)   >= 4: canvas1.create_line(raw_pts,   fill="red")
    if len(blue_pts)  >= 4: canvas1.create_line(blue_pts,  fill="blue")
    if len(green_pts) >= 4: canvas1.create_line(green_pts, fill="green")

def _aggregate_means_1min_per_pixel(history, width_px, group_size=60):
    """Return up to `width_px` mean values, each averaging 60 samples (≈1 minute at 1 Hz).
       Most-recent groups map to rightmost pixels; older to the left. """
    if width_px <= 0 or not history:
        return []
    need_samples = width_px * group_size
    src = history[-need_samples:]  # only what's needed
    points = []
    total = len(src)
    groups = (total + group_size - 1) // group_size
    start_group = max(0, groups - width_px)
    start_idx = start_group * group_size
    for g in range(start_group, groups):
        lo = (g - start_group) * group_size + start_idx
...
def draw_plot2():
    # Plot 2: 1 px = 1 minute (mean of 60 samples). Hour ticks every 60 px.
    canvas2.delete("all")
    w = max(canvas2.winfo_width(), 10); h = max(canvas2.winfo_height(), 10)

    # Hour ticks: every 60 pixels (60 min)
    for x in range(0, w, 60):
        canvas2.create_line(x, 0, x, h-1, fill="#e0e0e0")
    # darker line each 24 hours (1440 minutes)
    for x in range(0, w, 1440):
        canvas2.create_line(x, 0, x, h-1, fill="#c0c0c0")

    means = _aggregate_means_1min_per_pixel(STRIP_HISTORY, w, group_size=60)
    plot2_label.config(text="Plot 2: 1 px = 1 minute (mean of 60)   " + _format_hpa_label(means))
    if len(means) < 2:
        return

    n = len(means)
    if n >= w:
        xs = list(range(w - n, w))  # will be 0..w-1 when n==w
    else:
        xs = list(range(w - n, w))  # right-justified

    pmin = min(means); pmax = max(means); span = max(pmax - pmin, 1e-12)
    pts = []
    for x, v in zip(xs, means):
        y = h - 1 - ((v - pmin)/span)*(h-1)
        pts.extend((x, y))

    if len(pts) >= 4:
        canvas2.create_line(pts, fill="red")
...
def update():
    ...
    if len(STRIP_HISTORY) > 500000:  # retain plenty for long sessions
        del STRIP_HISTORY[:-500000]

    # temp stats
    if t_high_f is None or tF > t_high_f: t_high_f = tF
    if t_low_f  is None or tF < t_low_f:  t_low_f  = tF

    # labels (units with hPa suffix after numbers, as in prior request)
    abs_hpa.config(text=f"Absolute: {abs_h:7.3f} hPa")
    ...
    sm_lbl.config(text=("Smoothed: " + (f"{sm_val:.3f} hPa" if sm_val is not None else "-- hPa")))
    # NOAA label
    n60 = min(NOAA_SMOOTH_N, len(STRIP_HISTORY))
    noaa_val = sum(STRIP_HISTORY[-n60:])/n60 if n60 > 0 else None
    noaa_label.config(text=("NOAA Smoothed: " + (f"{noaa_val:.3f} hPa" if noaa_val is not None else "-- hPa")))

    rel_hpa.config(text=f"Relative: {rel_h:7.3f} hPa")
    ...
    if len(buf_1min) >= 2:
        ...
        slope_per_s = num/den
        tr_lbl.config(text=f"Trend: {slope_per_s*60.0:+.3f} hPa/min")
        avg = my
        nz = (sum((v-avg)**2 for v in y)/N) ** 0.5
        nz_lbl.config(text=f"Noise (RMS): {nz:.3f} hPa")
    else:
        tr_lbl.config(text="Trend: -- hPa/min")
        nz_lbl.config(text="Noise (RMS): -- hPa")
...
# ───── Initialize sensor again (safety) ─────
try:
    dps = init_dps310()
except Exception as e:
    messagebox.showerror("DPS310 Error", f"Failed to initialize DPS310:\n{e}\n\nThis build has no simulator fallback. Exiting.")
