...
# - Adalogger FeatherWing - RTC pcf8523 + SD
...
# - Adafruit JST-PH 2-pin Jumper Cable - 100mm long
...
operating_mode = TIME_ALT_inHg_TEMP
...
sd_spi = busio.SPI(board.SCK, board.MOSI, board.MISO)
...
if False:  # change to True if you want to set the time! False
    ...
    t = time.struct_time((2025, 5, 20, 20, 51, 0, 1, 140, 0))
...
# OLED display
    ...
    if operating_mode == TIME_ALT_inHg_TEMP:
        ...
        #    	print("Uptime: {:.2f} seconds".format(uptime))
        formatted_time = seconds_to_hms(uptime)
        #    	print("Formatted time:", formatted_time)
        print("TTO:", formatted_time)
        print(f"{get_battery_voltage():.2f} V, {battery_percentage():.0f} %")
        battery_voltage = get_battery_voltage()    # measured voltage in volts
        time_on_hours = uptime/3600        # device has been running for 2 hours
        temperature = 12         # measured temperature in °C
        time_remaining = estimate_remaining_time(battery_voltage, time_on_hours, temperature)
        print("Est time: {:.2f} hrs".format(time_remaining))
        ...
        if check_relay_status():
            NeoPixel_LED()
            print("Relay status: True")
        else:
            NeoPixel_LED((0,0,0))
            print("Relay status: False")
